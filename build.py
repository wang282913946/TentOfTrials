...
def commit_diagnostic_artifacts(paths: list[Path], commit_id: str) -> bool:
    ...
    if not existing:
        print(f"    {color('[X]', Colors.RED)} No diagnostic artifacts found to commit")
        return False
    ...
    if status.returncode != 0:
        print(f"    {color('[X]', Colors.RED)} Could not inspect diagnostic git status: {status.stderr.strip()}")
        return False
    if not status.stdout.strip():
        print(f"    {color('[OK]', Colors.GREEN)} Diagnostic artifacts already committed")
        return True
    ...
    if add.returncode != 0:
        print(f"    {color('[X]', Colors.RED)} Could not stage diagnostic artifacts: {add.stderr.strip()}")
        return False
    ...
    if commit.returncode != 0:
        ...
        print(f"    {color('[X]', Colors.RED)} Could not commit diagnostic artifacts: {output}")
        return False

    print(f"    {color('[OK]', Colors.GREEN)} Diagnostic artifacts committed")
    return True
...
def generate_logd(
    ...
    if encryptly_bin is None:
        ...
        print(f"    {color('[X]', Colors.RED)} {error}")
...
Diagnostic bundle:
  python3 build.py
    ...
    parser.add_argument(
        ...
        help="List modules with language, directory, and build command",
    )
    ...
    if args.list_modules:
        print(f"  {color('Module Reference:', Colors.BOLD)}")
        print(f"  {'Name':<25} {'Language':<12} {'Directory':<30} {'Build Command'}")
        print(f"  {'-'*25} {'-'*12} {'-'*30} {'-'*40}")
        for m in MODULES:
            build_short = ' '.join(m.build_cmd[:2])
            if len(m.build_cmd) > 2:
                build_short += "..."
            print(f"  {color(m.name, Colors.CYAN):<25} {m.language:<12} {str(m.dir.relative_to(ROOT)):<30} {build_short}")
        return 0
    ...
    if missing:
        ...
        print(f"  {color('[OK] All prerequisites found', Colors.GREEN)}")
    if args.module == "all":
        selected = MODULES
    else:
        names = [n.strip() for n in args.module.split(",")]
        # Validate ALL module names before proceeding (#225 bounty)
        valid_names = {m.name for m in MODULES}
        invalid_names = [n for n in names if n not in valid_names]
        if invalid_names:
            print(f"  {color('[X] Invalid module(s):', Colors.RED)} {', '.join(invalid_names)}")
            print(f"\n  {color('Valid modules:', Colors.GREEN)}")
            for m in MODULES:
                print(f"    {color(m.name, Colors.CYAN)} ({m.language}) - {m.build_cmd[0]} {m.build_cmd[1] if len(m.build_cmd) > 1 else ''}")
            return 1
    ...
    if not encryptly_ok:
        ...
        print(f"  {color('[X] encryptly cannot run', Colors.RED)}")
        ...
        return 1
    print(f"  {color('[OK] encryptly runs', Colors.GREEN)}")
